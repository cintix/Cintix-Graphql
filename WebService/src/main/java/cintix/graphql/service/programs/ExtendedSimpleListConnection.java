...
@PublicApi
public class ExtendedSimpleListConnection<T> implements DataFetcher<ExtendedConnection<T>>, TrivialDataFetcher<ExtendedConnection<T>> {

    static final String DUMMY_CURSOR_PREFIX = "extended-cursor:";
    private final String prefix;
    private final List<T> data;

    public ExtendedSimpleListConnection(List<T> data, String prefix) {
        this.data = assertNotNull(data, () -> " data cannot be null");
        assertTrue(prefix != null && !prefix.isEmpty(), () -> "prefix cannot be null or empty");
        this.prefix = prefix;
    }

    public ExtendedSimpleListConnection(List<T> data) {
    ...
    @Override
    public ExtendedConnection<T> get(DataFetchingEnvironment environment) {
        ...
        if (first != null) {
            if (first < 0) {
                throw new RuntimeException(format("The page size must not be negative: 'first'=%s", first));
            }
            edges = edges.subList(0, first <= edges.size() ? first : edges.size());
        }
        if (last != null) {
            if (last < 0) {
                throw new RuntimeException(format("The page size must not be negative: 'last'=%s", last));
            }
    ...
    private ExtendedConnection<T> emptyConnection() {
        PageInfo pageInfo = new DefaultPageInfo(null, null, false, false);
        return new ExtendedConnection<>(Collections.emptyList(), pageInfo, 0);
    }
    ...
    private int getOffsetFromCursor(String cursor, int defaultValue) {
        ...
        try {
            ...
            throw new RuntimeException(format("The cursor is not in base64 format : '%s'", cursor), e);
        }
        String string = new String(decode, StandardCharsets.UTF_8);
        if (prefix.length() > string.length()) {
            throw new RuntimeException(format("The cursor prefix is missing from the cursor : '%s'", cursor));
        }
        try {
            return Integer.parseInt(string.substring(prefix.length()));
        } catch (NumberFormatException nfe) {
            throw new RuntimeException(format("The cursor was not created by this class  : '%s'", cursor), nfe);
        }
    }
